...
def main():
    ...
    if menu == "대시보드":
        ...
        tab1, tab2, tab3, tab4 = st.tabs(["회원 목록", "회원 검색", "회원 추가", "회원 수정"])
        ...
        with tab4:
            ...
            # 수정할 회원 선택
            all_members = db_manager.execute_query("""
            ...
            if all_members:
                member_options = {f"{m['name']} ({m['email']})": m['id'] for m in all_members}
                selected_member_name = st.selectbox("수정할 회원 선택", options=list(member_options.keys()))
                selected_member_id = member_options[selected_member_name] if selected_member_name else None
                
                if selected_member_id:
                    # 선택된 회원의 현재 정보 가져오기
                    current_member = next(m for m in all_members if m['id'] == selected_member_id)
                    
                    st.write("---")
                    st.write("**현재 정보:**")
                    col1, col2 = st.columns(2)
                    with col1:
                        st.write(f"**이름:** {current_member['name']}")
                        st.write(f"**이메일:** {current_member['email']}")
                    with col2:
                        st.write(f"**전화번호:** {current_member['phone']}")
                        st.write(f"**상태:** {current_member['status']}")
                    st.write(f"**주소:** {current_member['address']}")
                    ...
                    with st.form("update_member"):
                        ...
                        with col1:
                            new_name = st.text_input("이름", value=current_member['name'])
                            new_email = st.text_input("이메일", value=current_member['email'])
                        
                        with col2:
                            new_phone = st.text_input("전화번호", value=current_member['phone'] or "")
                            new_status = st.selectbox("상태", 
                                                    options=["active", "inactive"],
                                                    index=0 if current_member['status'] == 'active' else 1,
                                                    format_func=lambda x: "활성" if x == "active" else "비활성")
                        
                        new_address = st.text_area("주소", value=current_member['address'] or "")
                        ...
                        with col1:
                            if st.form_submit_button("정보 수정", type="primary"):
                                if new_name and new_email:
                                    # 이메일 중복 체크 (본인 제외)
                                    email_check = db_manager.execute_query(
                                        "SELECT id FROM members WHERE email = ? AND id != ?",
                                        (new_email, selected_member_id)
                                    )
                                    ...
                                    if email_check:
                                        ...
                                        # 회원 정보 업데이트
                                        if member_manager.update_member(
                                            selected_member_id,
                                            name=new_name,
                                            email=new_email,
                                            phone=new_phone,
                                            ...
                                            # 상태 업데이트 (별도 처리)
                                            if new_status != current_member['status']:
                                                if new_status == 'active':
                                                    member_manager.activate_member(selected_member_id)
                                                else:
                                                    member_manager.deactivate_member(selected_member_id)
                        ...
                        with col2:
                            ...
                            if current_member['status'] == 'active':
                                if st.form_submit_button("회원 비활성화", type="secondary"):
                                    if member_manager.deactivate_member(selected_member_id):
                                        st.warning("회원이 비활성화되었습니다.")
                                        st.rerun()
                                    else:
                                        st.error("비활성화에 실패했습니다.")
                            else:
                                if st.form_submit_button("회원 활성화", type="secondary"):
                                    if member_manager.activate_member(selected_member_id):
                                        st.success("회원이 활성화되었습니다.")
                                        st.rerun()
                                    else:
                                        st.error("활성화에 실패했습니다.")
                        
                        with col3:
                            # 회원 삭제 버튼 (위험한 작업)
                            if st.form_submit_button("회원 삭제", type="secondary"):
                                # 대출 이력 확인
                                loan_history = db_manager.execute_query(
                                    "SELECT COUNT(*) as count FROM loans WHERE member_id = ?",
                                    (selected_member_id,)
                                )[0]['count']
                                
                                if loan_history > 0:
                                    st.error("대출 이력이 있는 회원은 삭제할 수 없습니다. 비활성화를 사용하세요.")
                                else:
                                    if member_manager.delete_member(selected_member_id):
                    ...
                    member_loans = db_manager.execute_query("""
                        SELECT l.*, b.title, b.author,
                               CASE WHEN l.status = 'active' AND l.due_date < date('now') THEN 1 ELSE 0 END as is_overdue
                        FROM loans l
                        JOIN books b ON l.book_id = b.id
                        WHERE l.member_id = ?
                        ORDER BY l.loan_date DESC
                    ...
                    if member_loans:
                        for loan in member_loans:
                            status_color = "🚨" if loan['is_overdue'] else ("📖" if loan['status'] == 'active' else "✅")
                            status_text = "연체" if loan['is_overdue'] else ("대출 중" if loan['status'] == 'active' else "반납 완료")
                            
                            st.write(f"{status_color} **{loan['title']}** by {loan['author']} ({status_text})")
                            if loan['status'] == 'active':
                                st.write(f"   대출일: {loan['loan_date']} | 반납예정일: {loan['due_date']}")
                            else:
                                st.write(f"   대출일: {loan['loan_date']} | 반납일: {loan['return_date']}")
