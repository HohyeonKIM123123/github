...
# 이미지 로드 - 실제 이미지 경로 없으면 None으로 설정
def load_image_safe(path):
    try:
        return pygame.image.load(path)
    except Exception:
        return None
background = load_image_safe(r"C:\Users\ghgh2\OneDrive\바탕 화면\github\02_pyGame\Nature Landscapes Free Pixel Art\nature_2\origbig.png")
hero = load_image_safe(r"C:\Users\ghgh2\OneDrive\바탕 화면\github\02_pyGame\화면 캡처 2025-07-14 141456.jpg")
enemy = load_image_safe(r"C:\Users\ghgh2\OneDrive\바탕 화면\github\orc_attack_frames\frame_1.png")
# arrow_imgs에 이미지가 없으면 None, 나중에 텍스트로 대체 출력
arrow_imgs = {
    'up': load_image_safe("assets/arrow_up.png"),
    'down': load_image_safe("assets/arrow_down.png"),
    'left': load_image_safe("assets/arrow_left.png"),
    'right': load_image_safe("assets/arrow_right.png")
}
perfect_img = load_image_safe("assets/perfect.png")
good_img = load_image_safe("assets/good.png")
bad_img = load_image_safe("assets/bad.png")
arrow_shot_img = load_image_safe(r"C:\Users\ghgh2\OneDrive\바탕 화면\github\02_pyGame\Tiny RPG Character Asset Pack v1.03 -Free Soldier&Orc\Arrow(Projectile)\Arrow01(32x32).png")
# 위치
...
# 방향키 시퀀스 그리기 (이미지 없으면 텍스트로 대체)
def draw_arrow_sequence(sequence, pos):
    x, y = pos
    for idx, key in enumerate(sequence):
        img = arrow_imgs.get(key)
        if img:
            screen.blit(img, (x + idx * 50, y))
        else:
            # 이미지 없으면 텍스트로 표시 (예: 'up', 'left' 등)
            draw_text(key, (x + idx * 50, y), BLACK, 36)
# 입력 결과 표시 함수 (이미지 없으면 텍스트로 대체)
def draw_result_image(result):
    ...
    if img:
        ...
        # 이미지 없으면 텍스트로 표시
        draw_text(result.upper(), pos, BLACK, 72)
# 게임 루프
running = True
...
while running:
    ...
    # 방향키 시퀀스 그리기
    draw_arrow_sequence(input_sequence, arrow_box_pos)
    # 입력 결과 표시
    if arrow_result and time.time() - show_arrow_result_time < 1.5:
        draw_result_image(arrow_result)
    # 화살 발사 애니메이션
    if arrow_shot_active:
        if arrow_shot_img:
            screen.blit(arrow_shot_img, (arrow_shot_x, arrow_shot_y))
        else:
            # 이미지 없으면 화살 표시 안함 (또는 텍스트)
            draw_text("->", (arrow_shot_x, arrow_shot_y), BLACK, 48)
